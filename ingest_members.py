...
def ingest_members(csv_path):
    ...
    Reads CSV, validates data, creates users and related records.
    """
    User = get_user_model()
    ...
    # Cache for subcounties and schools
    subcounty_cache = {}
    school_cache = {}
    ...
    # Create any missing subcounties
    for valid_sc in VALID_SUBCOUNTIES:
        key = valid_sc.lower()
        if key not in existing_subcounties:
            sc = SubCounty.objects.create(name=valid_sc)
            existing_subcounties[key] = sc
            print(f"  Created subcounty: {valid_sc}")
